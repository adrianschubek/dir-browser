...
const DEFAULT_PUBLIC_FOLDER = path.resolve(REPO_ROOT)
const DEFAULT_COUNTER_SNAPSHOT = path.resolve(
  REPO_ROOT,
  ".data",
  "counters.sqlite.bin"
)
...
const config = {
  ...
  publicFolder: path.resolve(
    process.env.PUBLIC_FOLDER ?? DEFAULT_PUBLIC_FOLDER
  ),
  ...
  readmeName: (
    process.env.README_NAME ?? "readme.md;readme.txt;readme.html;readme"
  )
    .split(";")
  ...
  authCookieLifetime: parseNumber(
    process.env.AUTH_COOKIE_LIFETIME,
    60 * 60 * 24 * 30
  ),
  ...
  searchEngines: (process.env.SEARCH_ENGINE ?? "s,g")
    .split(",")
  ...
  batchCompressAlgo: (
    process.env.BATCH_ZIP_COMPRESS_ALGO ?? "STORE"
  ).toUpperCase(),
  ignorePatterns: (process.env.IGNORE ?? "").split(";").filter(Boolean),
  port: parseNumber(process.env.PORT, 8080),
  counterSnapshotPath: path.resolve(
    process.env.COUNTER_SNAPSHOT_PATH ?? DEFAULT_COUNTER_SNAPSHOT
  ),
  counterFlushIntervalMs: parseNumber(
    process.env.COUNTER_FLUSH_INTERVAL_MS,
    10_000
  ),
}
...
async function readAccessConfig(
  ...
  if (
    typeof source.password_hash === "string" &&
    source.password_hash.length > 0
  ) {
    configValue.password_hash = source.password_hash
  }
  if (
    typeof source.password_raw === "string" &&
    source.password_raw.length > 0
  ) {
...
async function effectiveAccessForDir(
  localDirPath: string
): Promise<AccessState> {
  ...
  effective.requiresPassword = Boolean(
    effective.passwordHash || effective.passwordRaw
  )
  ...
  if (contentType.includes("application/json")) {
    try {
      ...
      return {
        ...
        downloadBatch: Array.isArray(batch)
          ? batch.filter((entry): entry is string => typeof entry === "string")
          : undefined,
      }
    } catch {
      return {}
    ...
    return {
      key:
        typeof formData.get("key") === "string"
          ...
          : undefined,
      downloadBatch: fieldBatch.filter(
...
function extractAccessKey(
  ...
  if (cookies.dir_browser_key)
    return { value: cookies.dir_browser_key, source: "cookie" }
...
async function accessStatusForLocalPath(
  localPath: string,
  key: string | null
): Promise<AccessState> {
  const localDir = (await fsp.stat(localPath)).isDirectory()
    ? localPath
  ...
  if (effective.requiresPassword) {
    if (!key) {
      ...
      authorized = await verifyPasswordCandidate(
        key,
        effective.passwordRaw,
        effective.passwordHash
      )
    }
  }
...
async function readMetadata(
  localPath: string
): Promise<DirectoryItem["metadata"] | null> {
  ...
  if (Array.isArray(source.labels)) {
    result.labels = source.labels.filter(
...
async function availablePath(
  ...
  if (!includeProtected && access.requiresPassword && !access.authorized)
    return null
...
class CounterStore {
  ...
  constructor(
    ...
    this.getStmt = this.db.prepare<{ count: number }, [string]>(
      "SELECT count FROM counters WHERE path = ?"
    )
    this.setStmt = this.db.prepare(
      "INSERT INTO counters(path, count) VALUES(?, ?) ON CONFLICT(path) DO UPDATE SET count=excluded.count"
    )
    this.insertStmt = this.db.prepare(
      "INSERT OR IGNORE INTO counters(path, count) VALUES(?, 0)"
    )
    this.incrementStmt = this.db.prepare(
      "UPDATE counters SET count = count + 1 WHERE path = ?"
    )
  ...
  private loadDatabase(): Database {
    try {
      if (fs.existsSync(this.snapshotPath)) {
        ...
        if (file.size > 0) {
          return Database.deserialize(fs.readFileSync(this.snapshotPath), {
            strict: true,
          })
        }
      }
      ...
      console.error(
        "[counter-store] failed to load snapshot, using empty in-memory db",
        error
      )
    }
...
const counterStore = new CounterStore(
  config.counterSnapshotPath,
  config.counterFlushIntervalMs
)
...
function withCorsHeaders(headers: Headers) {
  if (config.corsAllowAnyOrigin) {
    ...
    headers.set(
      ...
      "Authorization, Content-Type, X-Key"
    )
...
function inferPreviewKind(
  mimeType: string,
  extension: string
): "text" | "json" | "markdown" | "image" | "pdf" | "video" | "audio" | "none" {
...
async function readDirectoryReadmeHtml(
  localDirPath: string
): Promise<string | null> {
  ...
  try {
    const names = await fsp.readdir(localDirPath)
    readmeFilename =
      names.find((name) => config.readmeName.includes(name.toLowerCase())) ??
      null
  } catch {
    return null
  }
  ...
  const localReadmePath = path.join(localDirPath, readmeFilename)
  try {
...
async function listDirectory(
  ...
  key: string | null,
  includeReadme = true
): Promise<{ items: DirectoryItem[]; readmeHtml: string | null }> {
  ...
  for (const entry of entries) {
    ...
    const available = await availablePath(
      userPathFromLocal(localEntryPath),
      key
    )
    ...
    if (item.type === "dir") {
      ...
      item.auth_locked =
        available.access.requiresPassword && !available.access.authorized
      folders.push(item)
    } else {
      files.push(item)
    }
  ...
  const readmeHtml = includeReadme
    ? await readDirectoryReadmeHtml(localPath)
    : null
...
async function runSearch(
  ...
  async function maybePushResult(resultPath: string) {
    ...
    const availability = await availablePath(
      userPathFromLocal(resultPath),
      key,
      true
    )
  ...
  if (engine === "g") {
    ...
    for await (const match of glob.scan({ cwd: localPath })) {
      ...
      const resolved = await resolveExistingLocalPath(
        userPathFromLocal(path.join(localPath, match))
      )
...
async function getDeepUrlsFromArray(
  urls: string[],
  key: string | null
): Promise<string[]> {
  const files: string[] = []
  ...
  async function recurse(userPath: string) {
    ...
    if (stat.isDirectory()) {
      ...
      for (const entry of entries) {
        await recurse(
          path.posix.join(userPathFromLocal(available.localPath), entry)
        )
      }
      return
    }
...
async function createZipStreamResponse(
  filesToZip: Array<{ archiveName: string; localPath: string; size: number }>
): Promise<Response> {
...
async function getHash(localPath: string): Promise<string | null> {
  try {
    const hash = new Bun.CryptoHasher(
      config.hashAlgo as Bun.SupportedCryptoAlgorithms
    )
...
async function handleDirectoryRequest(args: {
  ...
  if (url.searchParams.has("readme") && !url.searchParams.has("ls")) {
    if (!config.apiEnabled) {
      return textResponse("API disabled", 404)
    }

    const readmeHtml = await readDirectoryReadmeHtml(localPath)
    return jsonResponse({
      path: userPath,
  ...
  if (config.batchEnabled && req.method === "POST") {
    ...
    if (batchUrls.length > 0) {
      ...
      if (unique.length === 0) {
        return textResponse(
          "Batch download error: no eligible files selected",
          400
        )
      }
      ...
      for (const selectedPath of unique) {
        ...
        if (!available) {
          return textResponse(
            `Batch download error: invalid file ${selectedPath}`,
            400
          )
        }
        ...
        if (stat.size > config.batchMaxFileSizeMb * 1024 * 1024) {
          return textResponse(
            `Batch download error: file exceeds per-file size limit: ${selectedPath}`,
            400
          )
        }
        ...
        if (totalSize > config.batchMaxTotalSizeMb * 1024 * 1024) {
          return textResponse(
            "Batch download error: total size exceeds configured limit",
            400
          )
        }
  ...
  if (url.searchParams.has("ls") || !isLegacyPath) {
    ...
    const includeReadme = url.searchParams.get("readme") !== "0"
...
async function handleFileRequest(args: {
  ...
  if (
    ...
    (config.hashRequired ||
      ...
      metadata?.hash_required)
  ) {
    const stat = await fsp.stat(localPath)
    if (!hashFileAllowed(stat.size)) {
      return textResponse(
        "Hashing disabled: file exceeds HASH_MAX_FILE_SIZE_MB",
        413
      )
    }
