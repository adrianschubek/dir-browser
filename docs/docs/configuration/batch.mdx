...
:::tip Large archive stability
For very large batch streams behind a reverse proxy, make sure upstream/client timeouts are high enough (for example `fastcgi_read_timeout` and `send_timeout`) to prevent truncated ZIP responses.

`BATCH_ZIP_COMPRESS_ALGO=STORE` is recommended for large already-compressed files (e.g. `.zip`, `.mp4`, `.jpg`) because it streams with lower CPU overhead.
:::
