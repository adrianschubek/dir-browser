...
Internally, each pattern is evaluated with [preg_match](https://www.php.net/manual/en/function.preg-match.php) against the browser-visible URL path. Matching is case-insensitive.
If any pattern matches, the file/folder will be hidden.

Notes:
- The path always starts with `/`, which represents the configured [public root](./public-root.md).
- Canonical filesystem paths and implementation details such as git-sync's worktree directory names are never matched.
- Patterns are wrapped as `#<pattern>#im` internally. Avoid using `#` in patterns.
- Invalid regular expressions are logged and skipped.
