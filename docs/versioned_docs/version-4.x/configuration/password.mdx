...
You can generate one using the following command or use an online generator. You need to escape `$` with [`$$`](https://docs.docker.com/reference/compose-file/interpolation/) in the env variable.
```bash
...
The entire dir-browser instance can be protected with a global password using either [basic auth](https://en.wikipedia.org/wiki/Basic_access_authentication) or a simple URL key.

### Definition

import EnvConfig from '@site/src/components/EnvConfig';

<EnvConfig name="PASSWORD_USER|PASSWORD_RAW|PASSWORD_HASH|PASSWORD_URL_KEY|AUTH_COOKIE_LIFETIME|AUTH_COOKIE_HTTPONLY" init="<empty>|<empty>|<empty>|<empty>|604800|true" values="<string>|<string>|<hash>|<string>|seconds|bool" desc="Username|Plaintext password|Hash of a password. See definition above.|Global URL auth key used as ?auth=<key>.|Lifetime of authentication cookie|Cookie HttpOnly flag" versions="3.3|3.3|3.3|4.5|4.2|4.2"/>
...
:::info
If `PASSWORD_URL_KEY` is set, requests must include `?auth=<PASSWORD_URL_KEY>`.
When browsing in the UI, dir-browser automatically appends the `auth` parameter to internal links.
If both basic auth and `PASSWORD_URL_KEY` are configured, both checks apply.
:::
...
Set `PASSWORD_URL_KEY` and access dir-browser with:

```
https://dir-demo.adriansoftware.de/?auth=foobar
```

You can also call APIs with the same query parameter:

```
curl "https://dir-demo.adriansoftware.de/examples/burger.jpg?auth=foobar"
```

Prefer HTTPS when using URL keys because query strings can appear in logs/history.
